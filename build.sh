...
if [ "$1" = "--linux-arch" ]; then
  ...
  sudo cp -r SFMP sfmp-style sfmp-icons /usr/local/bin
  ...
  sudo cp -r SFMP sfmp-style sfmp-icons /usr/local/lib

  echo "SFMP: player was best build, for launch: SFMP"

elif [ "$1" = "--android-termux" ]; then
  pkg install -y clang gtkmm3 pkg-config sfml taglib

  clang++ src/main.cpp -o SFMP $(pkg-config --cflags --libs gtkmm-3.0 taglib) -lsfml-audio -lsfml-system

  chmod +x SFMP

  cp -r SFMP sfmp-style sfmp-icons $PREFIX/bin
